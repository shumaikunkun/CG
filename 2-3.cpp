...
void display(void) {
	...
	glRotated(rotateAngle1, 0, 0, 1);  //黒い星の公転

	glPushMatrix();  //(衛星を追加する場合に備えてpushしておく)
	glColor3d(0.0, 0.0, 0.0);  //黒い小さい星
	glTranslated(0.3, 0, 0);
	glScaled(0.2, 0.2, 1.0);
	glRotated(rotateAngle1, 0, 0, 1);  //黒い星の自転
	glCallList(ID_DRAW_STAR);
	glPopMatrix();  //(衛星を追加する場合に備えてpopしておく)
	glPopMatrix();
